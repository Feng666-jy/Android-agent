...
function buildUpdateSets(cfg: any, data: Record<string, any>): { sets: string[]; params: any[] } {
  const sets: string[] = []
  const params: any[] = []
...
function updateOp(model: string, args: AnyArgs): any {
  ...
  const built = buildUpdateSets(cfg, data)
  const sets = built.sets
  const values = [...built.params, ...where.params]
...
function updateManyOp(model: string, args: AnyArgs): { count: number } {
  ...
  if (cfg.fields.updatedAt) data.updatedAt = nowIso()
  const built = buildUpdateSets(cfg, data)
  if (built.sets.length === 0) return { count: 0 }

  let sql = `UPDATE "${cfg.table}" SET ${built.sets.join(', ')}`
