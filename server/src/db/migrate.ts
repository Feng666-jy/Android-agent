 ...
 * - 本模块提供真正的迁移能力：按 server/src/db/migrations/<序号>_<名称>/migration.sql
...
export interface MigrationOptions {
  /** 迁移目录（默认 process.cwd()/server/src/db/migrations） */
...
export const DEFAULT_MIGRATIONS_DIR = path.resolve(process.cwd(), 'server/src/db/migrations')
...
export function appliedMigrationNames(db: DatabaseSync): Set<string> {
  const rows = db.prepare(`SELECT name FROM "_migrations" ORDER BY name`).all() as Array<{
    name: string
  }>
  return new Set(rows.map(row => row.name))
}
