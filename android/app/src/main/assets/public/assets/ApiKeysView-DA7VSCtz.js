...
import { r as b } from './request-eyLhzUlJ.js'
const A = {
    list() {
  ...
  z = { class: 'api-keys-view' },
  O = { key: 0, class: 'api-keys-view__error' },
    ...
    setup(d) {
      ...
      async function w() {
        ...
        try {
          ...
          x.value = ((a = e.data) == null ? void 0 : a.items) ?? []
        } catch (e) {
      ...
      async function S(a) {
        if (window.confirm(`确认吊销「${a.name}」？使用该 Key 的集成将立即失效。`))
          try {
            ;(await A.revoke(a.id), g('已吊销'), await w())
          } catch (e) {
      ...
      async function B(a) {
        if (u.value.name.trim())
          try {
            ;(await A.update(a.id, { name: u.value.name.trim(), scope: u.value.scope }),
              (f.value = null),
              g('已保存'),
              await w())
          } catch (e) {
      ...
      return (a, e) => (
        ...
        n('div', z, [
          ...
          v.value ? (l(), n('div', O, o(v.value), 1)) : p('', !0),
          ...
          k.value
            ? (l(),
              n('div', J, [
                ...
                s('button', { class: 'api-keys-view__btn-primary', onClick: D }, '创建')
              ]))
            : p('', !0),
          y.value
            ? (l(),
              n('div', Q, [
                ...
                s('div', W, [
                  s(
                    'button',
                    { class: 'api-keys-view__btn-primary', onClick: M },
                      ...
                      ? (l(),
                        n('div', te, [
                          ...
                          s('div', ae, [
                            s(
                              'button',
                              { class: 'api-keys-view__btn-primary', onClick: c => B(t) },
                              '保存',
                              8,
                              ie
                            ),
                        ...
                        n('div', ne, [
                          t.status === 'active' && !C(t)
                            ? (l(),
                              n(
                                'button',
                                { key: 0, class: 'api-keys-view__btn-ghost', onClick: c => U(t) },
                                ' 编辑 ',
                                8,
                                le
                              ))
                          ...
                          t.status === 'active'
                            ? (l(),
                              n(
                                'button',
                                { key: 1, class: 'api-keys-view__btn-danger', onClick: c => S(t) },
                                ' 吊销 ',
                                8,
                                oe
                              ))
  ...
  pe = q(ue, [['__scopeId', 'data-v-6d25e01b']])
